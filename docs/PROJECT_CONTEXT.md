...
## 1. Project Purpose
A deterministic, probability-based Vedic astrology system. It extracts data from classical horoscope PDFs, calculates explainable strengths, and layers timing modifiers to determine event manifestation probabilities. **It does NOT use AI to "guess" or "invent" astrological predictions.**

## 2. Current Architecture Flow
Data flows unidirectionally through stateless modules:
1. **PDF Extractor** -> Scrapes tables and text into raw dicts.
2. **JSON Normalizer** -> Cleans data, normalizes terminology, injects safe defaults.
...
6. **Pipeline Runner** -> Orchestrates data hand-offs sequentially.
7. **Unified JSON Output** -> The explainable, standardized engine payload.
