...
**Purpose:** This document is the persistent, long-term implementation memory for the project. It serves as the primary stabilization reference for AI-assisted development to ensure architectural continuity.
...
*   **The Immutable D1 Rule:** The D1 chart is the foundation of karma. Engines evaluating Vargas, Dashas, or Transits must *never* overwrite the D1 `final_score` or `breakdown`.
*   **Universal JSON Contract:** Every engine MUST return exactly: `metadata`, `final_score`, `breakdown`, `modifiers`, `temporal_activation`, and `confidence_flags`.
...
*   **Structural Modifiers (Capacity):** Vargas, Yogas, and Ashtakavarga evaluate the *quality* of the chart. These are mathematically **additive** (e.g., +15 points) and live in the `modifiers` object.
*   **Temporal Modifiers (Activation):** Dashas and Transits dictate *when* an event triggers. These are mathematically **multiplicative** (e.g., 1.25x) and live in the `temporal_activation` object.
...
## 6. Dasha & Transit Philosophy (Upcoming)
...
## 12. Future Caution Areas (DO NOT DO THIS)
...
## 13. Current Stable Import / Execution Rules
